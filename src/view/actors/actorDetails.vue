<template>
  <div class="container mx-auto py-16 flex">
    ...
    <div class="ml-20">
      ...
      >
        <div class="mt-4" v-for="movie in this.knownFor" :key="movie.id">
          <router-link
...
<script>
import { getPersonDetails } from "../../services/api.service.js";
import { getPersonDetailsCredits } from "../../services/api.service.js";
export default {
  data() {
    return {
      actor: {},
  ...
  mounted() {
    this.getPersonDetail(this.$route.params.id);
    this.getPersonDetailsCredits(this.$route.params.id, "combined_credits");
  },
  methods: {
    async getPersonDetail(person_id) {
      try {
        const response = await getPersonDetails(person_id);
        this.actor = response.data;
        this.knownFor = response.data.cast.slice(
          Math.max(response.data.cast.length - 5, 1)
        );
        //console.log('hi', this.actor);
      } catch (error) {
        console.log(error);
      }
    },
    async getPersonDetailsCredits(person_id, combined_credits) {
      try {
        const response = await getPersonDetailsCredits(
          person_id,
          combined_credits
        );
        this.knownFor = response.data.cast.filter(
          (x) => x.media_type == "movie"
        );
        //console.log("hi", this.actors.profile_path);
        console.log("hi", this.actor.creadit);
      } catch (error) {
        console.log(error);
      }
    },
    movieImage(movie) {
      ...
      if (!posterPath) {
        return "https://via.placeholder.com/185 x 278";
      } else {
  ...
  computed: {
    posterPath() {
      if (this.actor.profile_path) {
        ...
        return "https://via.placeholder.com/300 x 450";
      }
    },
  },
};
